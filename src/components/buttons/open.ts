...
export default new Button({
    customId: "open",
    build: ({ channel, mentionable }: { channel: GuildChannel; mentionable: Role | User }) => [
        new ButtonBuilder()
    ...
    execute: async ({ interaction, args }) => {
        ...
        const mentionable =
            interaction.guild?.roles.cache.get(mentionableId) || interaction.guild?.members.cache.get(mentionableId);
