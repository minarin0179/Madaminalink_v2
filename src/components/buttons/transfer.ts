...
export default new Button({
  ...
  execute: async ({ interaction, args }) => {
    ...
    const destinations: GuildTextBasedChannel[] = args
      ...
      .filter(
        (channel): channel is NewsChannel | TextChannel | VoiceChannel =>
    ...
    const messages = (
      ...
      .filter((message) => {
        ...
        const customId =
          firstComponent && "components" in firstComponent
            ? "customId" in firstComponent.components[0]
              ? firstComponent.components[0].customId
              : undefined
            : undefined;
